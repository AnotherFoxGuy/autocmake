...
def fetch_modules(config, relative_path, download_directory):
    ...
    print_progress_bar(
        text="- assembling modules:", done=0, total=num_sources, width=30
    )
                ...
                if "source" in d:
                    for src in to_l(d["source"]):
                        ...
                        if "http" in src:
                            ...
                            dst = os.path.join(
                                download_directory, "autocmake_{0}".format(module_name)
                            )
                            fetch_url(src, dst)
                            file_name = dst
                        ...
                        with open(file_name, "r") as f:
                            ...
                            for _k2, _v2 in parsed_config.items():
                                if _k2 not in to_d(v):
                                    ...
                                    cleaned_config[_k2] = flat_add(
                                        cleaned_config[_k2], _v2
                                    )
...
def process_yaml(argv):
    ...
    if setup_script_name.lower() != "none":
        ...
        s = gen_setup(
            cleaned_config, default_build_type, relative_path, setup_script_name
        )
...
def main(argv):
    ...
    if len(argv) != 2:
        ...
        sys.stderr.write(
            "        which will be needed to configure and build the project:\n"
        )
        sys.stderr.write("        $ {0} --self\n\n".format(argv[0]))
        sys.stderr.write(
            "Step 2: Create CMakeLists.txt and setup script in PROJECT_ROOT:\n"
        )
